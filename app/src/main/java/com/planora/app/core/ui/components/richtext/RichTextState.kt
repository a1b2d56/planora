...
import com.planora.app.core.utils.stripMarkdown
...
class RichTextState {
    ...
    fun toggleStyle(type: SpanType) {
        if (selection.collapsed) {
            // Smart Paragraph Formatting: Bullet Point
            if (type == SpanType.BulletList) {
                val pos = selection.min
                val textBefore = text.substring(0, pos)
                val lastNewLine = textBefore.lastIndexOf('\n')
                val lineStart = lastNewLine + 1
                val lineContent = textBefore.substring(lineStart)
                ...
                val adjusted = _spans.map { span ->
                    val s = if (span.start >= insertPos) span.start + bullet.length else span.start
                    val e = if (span.end >= insertPos) span.end + bullet.length else span.end
                    RichSpan(s, e, span.type)
                }
                _spans.clear()
                _spans.addAll(adjusted)
                
                val newText = StringBuilder(text).insert(insertPos, bullet).toString()
                text = newText
                selection = TextRange(insertPos + bullet.length)
                _styleVersion++
                return
            }

            // Inline Formatting: Sticky user style for future typing
            _hasUserToggle = true
